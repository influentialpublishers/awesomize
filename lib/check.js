...
// reqIf : Function -> [ String ] -> a , Request -> Maybe String
const reqIf = (condFn) => (keys) => (value, request) => {

  const x = _.props(keys, request)

  if (value || _.not(condFn(_.compose(_.not, _.isNil))(x)))
    return null

  else return MSG.REQUIRED

};

// reqIfHasAny : [ String ] -> a , Request -> Maybe String
const reqIfHasAny = reqIf(_.any);

// reqIfHasAll : [ String ] -> a , Request -> Maybe String
const reqIfHasAll = reqIf(_.all);

// notReqIf : Function -> [ String ] -> a , Request -> Maybe String
const notReqIf = (condFn) => (keys) => (value, request) => {

  const x = _.props(keys, request)

  if (value || condFn(_.compose(_.not, _.isNil))(x))
    return null
...
const notReqIfHasAny = notReqIf(_.any);

// notReqIfHasAll : [ String ] -> a , Request -> Maybe String
const notReqIfHasAll = notReqIf(_.all);
